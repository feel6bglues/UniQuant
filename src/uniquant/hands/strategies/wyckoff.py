...
@lru_cache(maxsize=1)
def load_wyckoff_params():
    """从 config/trading.yaml 加载 WyckoffEngine 参数和置信度阈值"""
    p = Path(__file__).resolve().parents[2] / "config" / "trading.yaml"
    with open(p) as f:
        cfg = yaml.safe_load(f)
    w = cfg["strategies"]["wyckoff"]
    min_cl = w.get("min_confidence_level", "B")
    return (w["lookback_days"], w["weekly_lookback"], w["monthly_lookback"], min_cl)


def trade_wyckoff(df: pd.DataFrame, as_of_date: str, csi: pd.DataFrame = None,
                  cost_buy: Optional[float] = None, cost_sell: Optional[float] = None,
                  symbol: str = "", is_st: bool = False, mode: str = "backtest", **kwargs) -> Optional[Dict]:
    ...
    if len(av) < 100:
        return None
    try:
        lk, wk, mk, min_conf = load_wyckoff_params()
        eng = WyckoffEngine(lookback_days=lk, weekly_lookback=wk, monthly_lookback=mk, is_st=is_st)
        rpt = eng.analyze(av, symbol=symbol, period="日线", multi_timeframe=True)
    except Exception as e:
        logger.warning("Wyckoff analysis failed: %s", e)
        return None
    rr = rpt.risk_reward
    ...
    if rpt.signal.signal_type == "no_signal" or rpt.trading_plan.direction == "空仓观望":
        return None
    try:
        threshold = ConfidenceLevel[min_conf]
        signal_conf = rpt.signal.confidence
        conf_order = [ConfidenceLevel.D, ConfidenceLevel.C, ConfidenceLevel.B, ConfidenceLevel.A]
        if conf_order.index(signal_conf) < conf_order.index(threshold):
        ...
        if sd <= ad:
            if xcals:
                try:
                    ...
                    trading_days = cal.sessions_in_range(sd, ad).size
                except Exception:
                    trading_days = len(pd.bdate_range(sd, ad))
            else:
                trading_days = len(pd.bdate_range(sd, ad))
            if trading_days <= 3:
                return None
