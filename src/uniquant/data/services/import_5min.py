...
logger = get_logger('import_5min')

FZLINE_RECORD_SIZE = 32
DATE_BASE_YEAR = 2004
...
class TDX5MinImporter:
    """通达信5分钟线数据导入器"""

    def __init__(self, tdx_dir: Union[str, Path] = TDX_DIR, output_dir: Union[str, Path] = LAKE_QUOTES_DIR):
        self.tdx_dir = Path(tdx_dir)
        self.output_dir = Path(output_dir) / '5mins'
        self.output_dir.mkdir(parents=True, exist_ok=True)
        # 初始化状态管理器
        self.state_manager = ImportStateManager(self.output_dir)
        # 线程锁用于文件写入
        self._file_lock = threading.Lock()
        logger.info(f"TDX5MinImporter 初始化完成，输出目录: {self.output_dir}")

    def _parse_fzline_record(self, data: bytes) -> Optional[dict]:
        """解析单条5分钟线记录"""
        if len(data) < FZLINE_RECORD_SIZE:
            return None
    ...
    def read_tdx_5min(self, market: str, code: str) -> Tuple[Optional[pd.DataFrame], Optional[Path]]:
        """读取通达信5分钟线文件"""
        fzline_file = self.tdx_dir / 'vipdoc' / market / 'fzline' / f'{market}{code}.lc5'

        if not fzline_file.exists():
            logger.debug(f"5分钟线文件不存在: {fzline_file}")
            return None, None

        records = []
        try:
            with open(fzline_file, 'rb') as f:
                while True:
                    data = f.read(FZLINE_RECORD_SIZE)
                    if len(data) < FZLINE_RECORD_SIZE:
                        break

                    record = self._parse_fzline_record(data)
                    if record:
                        record['code'] = code
            ...
            if not records:
                logger.warning(f"5分钟线文件无有效数据: {fzline_file}")
                return None, fzline_file

            df = pd.DataFrame(records)
            df['datetime'] = pd.to_datetime(df['datetime'])
            return df, fzline_file

        except FileNotFoundError as e:
            logger.error(f"读取5分钟线文件失败，文件未找到: {fzline_file} - {e}")
            return None, fzline_file
        except PermissionError as e:
            logger.error(f"读取5分钟线文件失败，权限不足: {fzline_file} - {e}")
            return None, fzline_file
        except Exception as e:
            logger.error(f"读取5分钟线文件失败 {fzline_file}: {e}")
            return None, fzline_file
    ...
    def import_single_stock(self, code: str, market: str,
        ...
        """导入单只股票的5分钟线数据（线程安全）"""
        try:
            df, source_file = self.read_tdx_5min(market, code)
            if df is None or df.empty:
                logger.debug(f"无5分钟线数据: {code}.{market}")
                if counter:
            ...
            if self.state_manager.is_import_needed(code, market, source_file):
                ...
                logger.debug(f"5分钟线数据导入成功: {code}.{suffix}, 共 {len(df)} 条记录")
            else:
            ...
            logger.error(f"导入5分钟线数据失败 {code}.{market}: {e}")
            if counter:
                counter.increment_failed()
            return False
    ...
    def import_batch(self, codes: List[str], markets: List[str]) -> Tuple[int, int]:
        ...
        logger.info(f"开始批量导入5分钟线数据，共 {len(codes)} 只股票，线程数: {MAX_WORKERS}")
    ...
    def incremental_update(self, codes: List[str], markets: List[str]) -> Tuple[int, int]:
        ...
        stocks_to_update = []
        for code, market in zip(codes, markets):
            source_file = self.tdx_dir / 'vipdoc' / market / 'fzline' / f'{market}{code}.lc5'
            if self.state_manager.is_import_needed(code, market, source_file):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='导入通达信5分钟线数据到数据湖')
