...
class StateManager:
    ...
    def update_state(
        ...
        ref_date = self._parse_analysis_date(analysis_result)
        state.watch_status = self._determine_watch_status(state, reference_date=ref_date)
    ...
    @staticmethod
    def _parse_analysis_date(analysis_result) -> Optional[datetime]:
        try:
            return datetime.strptime(analysis_result.analysis_date, "%Y-%m-%d")
    ...
    def _calculate_freeze_until(
        ...
        # 检测到 Spring，设置冷冻期
        analysis_date = self._parse_analysis_date(analysis_result)
        if analysis_date is None:
            raise ValueError(
                f"Cannot parse analysis_date from analysis_result (analysis_date={getattr(analysis_result, 'analysis_date', 'N/A')!r})"
            )
    ...
    def _determine_watch_status(self, state: AnalysisState, reference_date: Optional[datetime] = None) -> str:
        """确定观察状态"""
    ...
    def is_in_freeze_period(self, state: AnalysisState, reference_date: Optional[datetime] = None) -> bool:
        """检查是否在冷冻期"""
