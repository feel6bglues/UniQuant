#!/usr/bin/env python3
"""共享工具 — Deep Verify V2 (2026-08-12 深度验证)。
...
def clean_pool(df: pd.DataFrame) -> pd.DataFrame:
    """clean 池: fwd_20d 非空 ∩ 剔 ETF ∩ 剔 000/399 指数前缀。"""
    out = df[df["fwd_20d"].notna()].copy()
    out = out[~out["is_etf"].fillna(False)].copy()
    out = out[~out["symbol"].str.startswith(INDEX_PREFIXES)].copy()
    return out
...
def write_out(name: str, payload: dict) -> Path:
    out = ROOT / "results" / "wyckoff_deep_verify"
