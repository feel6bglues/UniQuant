# -*- coding: utf-8 -*-
"""WS1 重测: 沪深300 全成分 (300 只) × 2010-2024 年报 -> 次年收益
IC/IR + 牛熊拆分 + 2024-02 压力测试
"""
import sys
import io
import os
...
panel_frames = []
for y in YEARS:
    df = c.fetch_report(f"gpcw{y}1231.zip")   # 已缓存, 本地解析
    if len(df):
...
print(f"财报面板: {panel.shape} (全A, 已含300只)")
...
for code in CODES:
    ...
    for y in YEARS:
        ...
        rows.append({
            ...
            "dy": year_dps(code, y) / p_end if p_end else np.nan,
...
print("因子覆盖:")
print(df[["roe", "ep", "fcfy", "dy", "np_gr"]].notna().mean().round(3).to_string())

# ---------- 逐期截面 rank IC ----------
factors = {"ROE": "roe", "EP": "ep", "FCF_Yield": "fcfy", "DY": "dy", "SUE(np_gr)": "np_gr",
           "现金含量": "cash_content"}
...
ic_df.to_csv("mx_fin_data/hs300_ic_matrix.csv", encoding="utf-8-sig")
print("\n=== 逐期 rank IC (300只) ===")
...
summ_df.to_csv("mx_fin_data/hs300_ic_summary.csv", index=False, encoding="utf-8-sig")
...
rdf.to_csv("mx_fin_data/hs300_ic_regime.csv", index=False, encoding="utf-8-sig")
...
for code in CODES:
    ...
    if len(r):
        stress.append({"code": code, "ret": p_feb / p_jan - 1,
                       "roe": r.iloc[0]["roe"], "fcfy": r.iloc[0]["fcfy"]})
sdf = pd.DataFrame(stress)
print(f"样本: {len(sdf)} 只 | 平均收益: {sdf['ret'].mean()*100:.2f}%")
for f in ["roe", "fcfy"]:
    ic = sdf[f].rank().corr(sdf["ret"].rank())
    print(f"  {f} rankIC(2024-02) = {ic:.4f}")
med = sdf["roe"].median()
print(f"  高ROE组 {sdf[sdf['roe']>med]['ret'].mean()*100:.2f}% vs 低ROE组 {sdf[sdf['roe']<=med]['ret'].mean()*100:.2f}%")
sdf.to_csv("mx_fin_data/hs300_stress_202402.csv", index=False, encoding="utf-8-sig")

df.to_csv("mx_fin_data/hs300_annual_panel.csv", encoding="utf-8-sig")
print("\nDONE")
